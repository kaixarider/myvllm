...
class ModelRunner:
    ...
    def load_model(self) -> None:
        ...
        #Singleton.model_weight=self.model_memory_usage / float(2**30)
    ...
    def execute_model(
        ...
        '''
        if decode_meta:
            Singleton.max_iteration=max(Singleton.max_iteration,len(sampling_metadata.__dict__['seq_groups']))
            if len(sampling_metadata.__dict__['seq_groups'])!=Singleton.max_iteration:
        ...
        # sample也应该加进去
        '''
        if prefill_meta and Singleton.record:
            for i in range(len(sampling_metadata.__dict__['seq_groups'])):
                Singleton.record[int(sampling_metadata.__dict__['seq_groups'][i].__dict__['seq_ids'][0])]["prefill"]+=duration
        if decode_meta and Singleton.record:
            for i in range(len(sampling_metadata.__dict__['seq_groups'])):
                Singleton.record[int(sampling_metadata.__dict__['seq_groups'][i].__dict__['seq_ids'][0])]["decode"]+=duration
        torch.cuda.synchronize()
        start=time.time()
        '''
        ...
        cp=output.__dict__['outputs']
        if len(cp)!=256:
            if prefill_meta:
                total_memory=0
                for i in cp:
                    key=i.__dict__['samples'][0].__dict__['parent_seq_id']    
                    Singleton.sample[key]['prefill']+=end-start
                
            if decode_meta:
                for i in cp:
                    key=i.__dict__['samples'][0].__dict__['parent_seq_id']
                    Singleton.sample[key]['decode']+=end-start
                    Singleton.iteration[key]=len(cp)
                if len(cp)!=Singleton.max_iteration:
                    Singleton.start=False
        '''
        return output

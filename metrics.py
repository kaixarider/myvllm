...
class StatLogger:
    ...
    def log(self, stats: Stats) -> None:
        ...
        if self._local_interval_elapsed(stats.now):
            ...
            '''
            Singleton.generation_throughput=max(Singleton.generation_throughput,generation_throughput)
            Singleton.prompt_throughput=max(Singleton.prompt_throughput,prompt_throughput)
            '''
            ...
            # Log to stdout.
            '''with open('/home/wangsiyuan/workspace/llm/result/outputs/stats-qwen-multi_news_e.txt', 'a+') as writers:
                str2="Avg prompt throughput:"+str(prompt_throughput)+"tokens/s,"+"Avg generation throughput:"+str(generation_throughput)+"tokens/s, "
                str3="Running:"+str(stats.num_running_sys)+"reqs, Swapped:"+str(stats.num_swapped_sys)+" reqs,"
                str4="Pending: "+str(stats.num_waiting_sys)+" reqs, GPU KV cache usage:"+ str(stats.gpu_cache_usage_sys*100) +"%,"
                str5="CPU KV cache usage:"+str(stats.cpu_cache_usage_sys*100)+"%."
                writers.write(str2)
                writers.write(str3)
                writers.write(str4)
                writers.write(str5)
            ...
            logger.info(
                ...
                "Pending: %d reqs, GPU KV cache usage: %.3f%%, "
                "CPU KV cache usage: %.3f%%.",
                prompt_throughput,

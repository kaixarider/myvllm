...
import ray
from vllm.ray_timer import metricstype,Timer
...
class GPUModelRunnerBase(ModelRunnerBase[TModelInputForGPU]):
    ...
    def load_model(self) -> None:
        ...
        actor1.set_value.remote(self.model_memory_usage / float(2**30),metricstype.model_weight)
...
class ModelRunner(GPUModelRunnerBase[ModelInputForGPUWithSamplingMetadata]):
    ...
    def execute_model(
        ...
        if decode_meta and sampling_metadata.__dict__['seq_groups']:
            worker1.max_value.remote(len(sampling_metadata.__dict__['seq_groups']),metricstype.max_iteration)
            worker2.max_value.remote(len(sampling_metadata.__dict__['seq_groups']),metricstype.max_iteration)
            itn1=ray.get(worker1.get_value.remote(metricstype.max_iteration))
            itn2=ray.get(worker2.get_value.remote(metricstype.max_iteration))
            bt=len(sampling_metadata.__dict__['seq_groups'])
            if itn1!=bt and itn2!=bt:
                ray.get(worker1.set_start_decode.remote(False))
                ray.get(worker2.set_start_decode.remote(False))
               
            else:
                ray.get(worker1.set_start_decode.remote(True))
                ray.get(worker2.set_start_decode.remote(True))
        ...
        with set_forward_context(model_input.attn_metadata):
            ...
            if prefill_meta and sampling_metadata.__dict__['seq_groups']:
                for i in range(len(sampling_metadata.__dict__['seq_groups'])):
                    ray.get(worker1.set_record_value.remote(int(sampling_metadata.__dict__['seq_groups'][i].__dict__['seq_ids'][0]),"prefill",duration))
            if decode_meta and sampling_metadata.__dict__['seq_groups']:
                for i in range(len(sampling_metadata.__dict__['seq_groups'])):
                    ray.get(worker1.set_record_value.remote(int(sampling_metadata.__dict__['seq_groups'][i].__dict__['seq_ids'][0]),"decode",duration))

...
from sglang.singleton import Singleton,sync_type
import torch
import zmq
import time
            ...
            if batch:
                if batch.forward_mode.is_decode():
                    if torch.cuda.current_device()==0:
                        if Singleton.is_prefilling:
                            Singleton.log_prefill('/home/wangsiyuan/workspace/llm/result/distributedllama/llamaprefill1.json')
                            Singleton.is_prefilling=False
                        else:
                            print(Singleton.decode_gemm)
                start=0
                end=0
                if torch.cuda.current_device() == 0:
                    if Singleton.is_prefilling:
                        iteration=Singleton.prefill_iteration
                        Singleton.prefill_attention[iteration]=0
                        Singleton.prefill_gemm[iteration]=0
                        torch.cuda.synchronize()
                        start=time.time()
                    else:
                        iteration=Singleton.decode_iteration
                        Singleton.decode_attention[iteration]=0
                        Singleton.decode_gemm[iteration]=0
                        torch.cuda.synchronize()
                        start=time.time()
                '''
                elif torch.cuda.current_device() == 1 and Singleton.is_prefilling:
                    Singleton.prefill_attention[iteration]=0
                    Singleton.prefill_gemm[iteration]=0'''
                ...
                if torch.cuda.current_device() == 0 :
                    if Singleton.is_prefilling:
                        ...
                        Singleton.prefill_time.append(end-start)
                    else:
                        torch.cuda.synchronize()
                        end=time.time()
                        Singleton.decode_time.append(end-start)
                        Singleton.decode_batchsize.append(len(batch.reqs))

                self.process_batch_result(batch, result)
                if torch.cuda.current_device() == 0 :
                    if Singleton.is_prefilling:
                        Singleton.prefill_iteration+=1
                    else:
                        Singleton.decode_iteration+=1    
    ...
    def get_new_batch_prefill(self) -> Optional[ScheduleBatch]:
        ...
        if self.tp_rank == 0:
            ...
            if num_mixed_running > 0:
                ...
                Singleton.prefill_token_num.append(adder.log_input_tokens)
        # Create a new batch
